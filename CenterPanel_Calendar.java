...
public class CenterPanel_Calendar 
{
	...
	protected void openOtherMonthList(calButton day)
	{
		...
		if(day.getCount() == 0)
		{
			...
			temp.setDayLabel("" + day.getMonth() + " " + day.getDay() + ", " + day.getYear());
			day.setList(temp);
		}
	...
	/**
	 * This sets the button for days of the current month
	 **/
	private void setMonthButtons()
	{
		...
		day.setDate(copyCalendar.get(copyCalendar.DATE), copyCalendar.get(copyCalendar.MONTH), copyCalendar.get(copyCalendar.YEAR));
	...
	/**
	 * This sets the button for days outside current month
	 **/
	private void setOutsideMonthButtons()
	{
		String temp = Integer.toString(copyCalendar.get(copyCalendar.DATE));
		calButton day = new calButton(temp);
		day.setDate(copyCalendar.get(copyCalendar.DATE), copyCalendar.get(copyCalendar.MONTH), copyCalendar.get(copyCalendar.YEAR));
	...
	protected void setCalendarGrid()
	{
		...
		//This sets the days before the current month.
				...
				|| copyCalendar.get(copyCalendar.MONTH) > tempMonth && copyCalendar.get(copyCalendar.YEAR) < myCalendar.get(myCalendar.YEAR))
		{
			...
			{
				setOutsideMonthButtons();
			}
			
			//This sets the days of the month
			else
			{
				setMonthButtons();
			}
		...
		//This sets the days after the month
